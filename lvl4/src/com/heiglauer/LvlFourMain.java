...
public class LvlFourMain {
    public static void main(String[] args) {
        String file = "lvl4/Files/level4_1";
        ...
        try(BufferedReader reader = new BufferedReader(new FileReader(file+".in"))){
            ...
            try {
                ...
                for (int tournamentCounter = 0; tournamentCounter < numberOftournaments; tournamentCounter++) {
                    ...
                    } while (doesScissorWin(fighterOrder, numberOfFighters));
    ...
    public static boolean doesScissorWin(char[] fighterOrder, int numberOfFighters){
        int numberOfRounds = log2(numberOfFighters);
        int nextRoundFighterCounter = 0;
        for(;;){
            char[] fightersInNextRound = new char[fighterOrder.length/2];
            for(int fighterPairCounter = 0; fighterPairCounter < fighterOrder.length; fighterPairCounter+=2){
                fightersInNextRound[nextRoundFighterCounter] = fighterOneWins(fighterOrder[fighterPairCounter],fighterOrder[fighterPairCounter+1]) ? fighterOrder[fighterPairCounter]:fighterOrder[fighterPairCounter+1];
                nextRoundFighterCounter++;
            }
            fighterOrder = new char[fightersInNextRound.length];
            fighterOrder = fightersInNextRound;
            nextRoundFighterCounter = 0;
            if(fighterOrder.length == 1)
                break;
        }
        if(fighterOrder[0] == 'S')
            return true;
